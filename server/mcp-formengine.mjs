...
const LIFECYCLE_TOOLS = [
  ...
  { name: 'save_form', description: 'PREVIEW / VIEW / OPEN / SHARE the current form — this is the default tool for any of those. Saves the form and returns a short, shareable link that works for forms of ANY size. Re-saving overwrites the same draft (stable link) until new_form. Prefer this over preview_url whenever the user wants to see, preview, open, share, or get a link to the form.', parameters: { type: 'object', properties: { name: { type: 'string', description: 'optional label for the draft' } } } },
  { name: 'preview_url', description: 'Low-level fallback: builds a link with the ENTIRE form embedded in the URL (no save, no backend). The URL gets very long and is truncated by chat for non-tiny forms. Only use for a quick throwaway check of a SMALL form, or when explicitly asked for an embedded/offline link. For normal preview/share, use save_form instead.', parameters: { type: 'object', properties: {} } },
